 ...
 * Body: { sellerId: number, reason: string }
 */
router.post("/", verifyToken, isAdmin, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ message: "A valid sellerId (number) and non-empty reason (string) are required." });
    }
    ...
    if (existingSeller.approvalStatus === "rejected") {
      console.warn(`Seller ID ${sellerId} already rejected. Updating rejection reason.`);
    }
    ...
    const updatedSellerResult = await db
      ...
      .set({
        approvalStatus: sql`'rejected'::seller_approval_status`, // ✅ enum-safe
    ...
    // 3. Update user role back to "customer"
    ...
    if (!updatedUser) {
      console.warn("User role not updated. UID:", existingSeller.userId);
    }
